...
from utils.dataset.wcn_systemAct import read_wcn_data, prepare_wcn_dataloader
...
def cal_total_loss(scores, batch_labels, opt):
    batch_size = scores.size(0)
    loss = opt.class_loss_function(scores, batch_labels)
    loss_record = loss.item() / batch_size
    return loss_record, loss


def pred_one_sample(score, memory, opt):
    label_ids = [j for j, p in enumerate(score) if p > 0.5]
    pred_classes = [memory['idx2label'][j] for j in label_ids]
    return pred_classes
...
def eval_epoch(model, data, opt, memory, fp, efp):
    ...
    for j, batch in enumerate(data):
        ...
        for i, (score, gold, raw) in enumerate(zip(scores.tolist(), raw_labels, raw_in)):
            pred_classes = pred_one_sample(score, memory, opt)
